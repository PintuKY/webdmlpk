...
$posted = array(
    'key' => $MERCHANT_KEY,
    'txnid' => $txnid,
    'productinfo' => $sellerService_id,
    'surl' => route('ecom.ecomPayyouPayment'),
    'furl' => url('paymentfailed'),
    'service_provider' => 'payu_paisa',
    'firstname' => $name.'-'.$landmark,
    'address1' => $address.'-'.$pincode,
    'email' => $email,
    'phone' => $phone,
    'udf1' => Auth::user()->id,
    'udf2' => $date,
...
<form class="col" method="post" action="{{$action}}" name="payuForm">
    ...
    <input type="hidden" name="firstname" value="{{$name.'-'.$landmark}}" />
    <input type="hidden" name="email" value="{{$email}}" />
    <input type="hidden" name="phone" value="{{$phone}}" />
    <input type="hidden" name="address1" value="{{$address.'-'.$pincode}}">
    <input type="hidden" name="key" value="<?php echo $MERCHANT_KEY ?>" />
    <input type="hidden" name="hash" value="<?php echo $hash ?>"/>
    <input type="hidden" name="txnid" value="<?php echo $txnid ?>" />
    <input type="hidden" name="productinfo" value={{$sellerService_id}}>
    ...
    <input type="hidden" name="udf2" value="{{$date}}"/>
    <input type="hidden" name="udf3" value="{{$time}}"/>
