        ...
        <div class="tz-regi-form">
            ...
            <div class="tab-content">
                <div id="home" class="tab-pane fade in active">
                    <form class="col s12" method="post" action="{{ route('customer.login') }}">
                        @csrf
                        @php $actual_link = @$_SERVER['HTTP_REFERER']; @endphp
                        @if(@$_REQUEST['return'] == "staybooking-checkout")
                        <?php 
                            $actual_link = url('/staybooking-checkout?from_date='.$_REQUEST['from_date'].'&to_date='.$_REQUEST['to_date'].'&adult='.$_REQUEST['adult'].'&hotel='.$_REQUEST['hotel'].'&adults='.$_REQUEST['adults'].'&children='.$_REQUEST['children'].'&rooms='.$_REQUEST['rooms'].'&childrenExtra='.$_REQUEST['childrenExtra'].'&adultExtra='.$_REQUEST['adultExtra'].'');
                        ?>
                        @endif
                        
                        <input type="hidden" name="return" value="{{$actual_link}}">
                        <div class="row">
                ...
                | Are you a new user ? <a href="{{ route('register') }}">Register</a> </p>
                </div>
