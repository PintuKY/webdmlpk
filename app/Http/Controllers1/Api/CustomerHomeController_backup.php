  ...
  $sellerService=SellerService::find($order->sellerService_id);
	...
	public function stayBookingSearch(Request $request,$id)
   {
		...
		if (isset($request->from_date) && $request->from_date!='' && isset($request->to_date) && $request->to_date!='' ) {
			...
			$total = 0;
			if(isset($request->adult) && isset($request->children)){
				$total = (array_sum($request->adult) + array_sum($request->children));
			}
			if(isset($request->adult) && !isset($request->children)){
				$total = array_sum($request->adult);
			}
			if(!isset($request->adult) && isset($request->children)){
				$total = array_sum($request->children);
			}
			...
			/*$roomsAdded=Room::find($request->search_id);
			if(!$roomsAdded){
            	return response()->json(['message'=>'Room not found','success'=>false,'statusCode'=>404]);
			}
			$orderRooms=OrderRoom::where('order_id',$roomsAdded->id)->get();
			$ch=[];
			$ad=[];
			foreach($orderRooms as $orderRoom){
			$ch[]=$orderRoom->children;
				$ad[]=$orderRoom->adult;
				$orderRoom->delete();
			}
			$children= $ch;
